...
const notifyFlag =($.isNode() ? process.env.fknotify : $.getdata('fknotify'))|| 0; 	//0为关闭通知，1为打开通知,默认为0

let tsxx='',tsflag=0	//推送信息
//   token 数据
let token = ($.isNode() ? process.env.token : $.getdata('token'));
let tokenArr=[
"1e43bd91-aee0-410a-a4e8-9c466a65521b",
]
...
async function bcc() {
    let url = 'https://s221.yyyyy.run/api/user/index'
let body=``
    let urlObject = populateUrlObject(url,body)
    await httpRequest('post',urlObject)
    let result = JSON.parse(httpResult.body);
    money=result.data.money
		console.log(`用户:${result.data.nickname}       可提余额: ${result.data.money}`)
}
...
async function acc() {
    let url = 'https://s221.yyyyy.run/api/sign/userSignIn'
let body=``
    let urlObject = populateUrlObject(url,body)
    await httpRequest('post',urlObject)
    let result = JSON.parse(httpResult.body);
 ...
 async function acc2() {
    let url = 'https://s221.yyyyy.run/api/sign/userSignData'
		
	let body=``
    let urlObject = populateUrlObject(url,body)
    await httpRequest('post',urlObject)
    let result = JSON.parse(httpResult.body);
		console.log(`总积分 ${result.data.coin} 今日积分 ${result.data.today_coin}`)
}

async function acc3() {
    let url = 'https://s221.yyyyy.run/api/user/postWith'
	let body=`num=5`
    let urlObject = populateUrlObject(url,body)
    await httpRequest('post',urlObject)
    let result = JSON.parse(httpResult.body);
...
async function acc4() {
    let url = 'https://s221.yyyyy.run/api/user/lingqu'
	let body=``
    let urlObject = populateUrlObject(url,body)
    await httpRequest('post',urlObject)
    let result = JSON.parse(httpResult.body);
...
function populateUrlObject(url,body=''){
    ...
    let urlObject = {
        url: url,
        headers: {"Accept-Encoding": "gzip,compress,br,deflate","Connection": "keep-alive","Content-Length": "5","Host": host,"Referer": "https://servicewechat.com/wx336f8bb6f95f891a/1/page-frame.html","User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.20(0x18001442) NetType/WIFI Language/zh_CN","content-type": "application/x-www-form-urlencoded","token":token},
    }
